...
CafeAI implements a **tiered memory hierarchy** with four rungs, each a deliberate choice
...
Rung 3 → Redis via Lettuce      (distributed, multi-instance)
Rung 4 → Hybrid (warm + cold)   (tiered promotion/demotion)
```
...
- Serialization format for off-heap storage must be chosen (FlatBuffers or a custom
  `MemoryLayout`) — see ADR-004.
